...
try{
	String sql="select to_char(orderdate,'YYYY-MM-DD'),addr,tel,pay,cardno,prodcount,total from order0804 where id=? and name=?";
	pstmt=conn.prepareStatement(sql);
	...
	if(rs.next()){
		...
		tel=rs.getString(3);
		pay=rs.getString(4);
		cardno=rs.getString(5);
		prodcount=rs.getString(6);
...
<br>
<br>
<h3 class="h3_center">주문정보 수정</h3>
<form name="form" method="post" action="update_order.jsp">
<table border="1" class="tab1">
<tr>
	<th class="th2">상품코드</th>
		<td>
			<input type="text" name="id"  value="<%=id%>"class="in2" onchange="ch()">
		</td>
...
<tr>
	<th class="th2">결제방법</th>
		<td>
			<input type="radio" name="pay" value="현금" <%=pay.equals("현금")?"checked":"" %> onclick="ch1()">현금
			<input type="radio" name="pay" value="카드" <%=pay.equals("카드")?"checked":"" %> onclick="ch1()">카드
		</td>
